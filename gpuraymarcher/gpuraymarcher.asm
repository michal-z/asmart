...
load_binary_file:
    ...
    .k_stack_size = $
        ...
        $invoke         CreateFile,rcx,dword GENERIC_READ,dword 0,0,dword OPEN_EXISTING,dword FILE_ATTRIBUTE_NORMAL,0
    ...
    @@: $test           rdi,rdi
        $jz             @f
        $invoke         HeapFree,[def_heap],dword 0,rdi
    @@: $xor            eax,eax
        $xor            edx,edx
    .done:
        $add            rsp,.k_stack_size
...
init:
    ...
    .k_stack_size = $
        ...
        $jz             .error
        ; window
        $invoke         AdjustWindowRect,win_rect,dword k_win_style,dword 0
    ...
    @@: ; device
        $invoke         D3D12CreateDevice,0,dword D3D_FEATURE_LEVEL_11_1,IID_ID3D12Device,device
        $test           eax,eax
        $js             .error
        ...
        $js             .error
        ; command allocator
        $comcallv       rdi,rsi,ID3D12Device.CreateCommandAllocator,dword D3D12_COMMAND_LIST_TYPE_DIRECT,IID_ID3D12CommandAllocator,cmdallocator
        $test           eax,eax
        $js             .error
        ; swap chain
        $mov            rax,[win_handle]
        ...
        $comcallv       rdi,rsi,ID3D12Device.GetDescriptorHandleIncrementSize,dword D3D12_DESCRIPTOR_HEAP_TYPE_RTV
        $mov            [rtv_inc_size],eax
        $comcallv       rdi,rsi,ID3D12Device.GetDescriptorHandleIncrementSize,dword D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV
    ...
    .for_each_swap_buffer:
        ...
        ; fence
        $comcallv       rdi,rsi,ID3D12Device.CreateFence,0,dword D3D12_FENCE_FLAG_NONE,IID_ID3D12Fence,fence
        $test           eax,eax
        $js             .error
        ; fence event
        ...
        $jz             .error
        ; root signature
        $invoke         D3D12SerializeRootSignature,root_signature_desc,dword D3D_ROOT_SIGNATURE_VERSION_1,d3dblob,0
        $comcall        [d3dblob],ID3DBlob.GetBufferPointer
        $mov            rbx,rax
        $comcall        [d3dblob],ID3DBlob.GetBufferSize
        $comcallv       rdi,rsi,ID3D12Device.CreateRootSignature,dword 0,rbx,rax,IID_ID3D12RootSignature,root_signature
        $mov            ebx,eax
        $comrelease     [d3dblob]
        $test           ebx,ebx
        $js             .error
        ; create command list
        $comcallv       rdi,rsi,ID3D12Device.CreateCommandList,dword 0,dword D3D12_COMMAND_LIST_TYPE_DIRECT,[cmdallocator],0,\
        ...
        $jmp            .return
    .error_cpu:
        $invoke         MessageBox,dword 0,_err_cpu_message,_err_cpu_caption,dword 0
        $xor            eax,eax
        $jmp            .return
    .error:
        $invoke         MessageBox,dword 0,_err_init_message,_err_init_caption,dword 0
        $xor            eax,eax
    .return:
        $add            rsp,.k_stack_size
        $pop            rbx rsi rdi
...
start:
    ...
    .k_stack_size = $+16
        ...
        $jz             .quit
    .main_loop:
        $invoke         PeekMessage,win_msg,0,dword 0,dword 0,dword PM_REMOVE
        $test           eax,eax
        $jz             .update
...
align 8
win_handle dq 0
win_title db 'GPU Raymarcher', 64 dup (0)
win_title_fmt db '[%d fps  %d us] GPU Raymarcher',0
