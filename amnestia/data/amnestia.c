...
static int init_audio(audio_state_t *audio)
{
  ...
  if (FAILED(CoCreateInstance(&CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL,
                              &IID_IMMDeviceEnumerator, (void **)&audio->enumerator))) return 0;
  if (FAILED(IMMDeviceEnumerator_GetDefaultAudioEndpoint(audio->enumerator, eRender, eConsole, &audio->device))) return 0;
  if (FAILED(IMMDevice_Activate(audio->device, &IID_IAudioClient, CLSCTX_ALL, NULL, (void **)&audio->client))) return 0;

  WAVEFORMATEX format = {
    .wFormatTag = WAVE_FORMAT_PCM, .nChannels = 2, .nSamplesPerSec = 44100, .nAvgBytesPerSec = 44100*4, .nBlockAlign = 4,
    .wBitsPerSample = 16
  };
  if (FAILED(IAudioClient_IsFormatSupported(audio->client, AUDCLNT_SHAREMODE_EXCLUSIVE, &format, NULL))) return 0;

  REFERENCE_TIME period;
  if (FAILED(IAudioClient_GetDevicePeriod(audio->client, NULL, &period))) return 0;

  HRESULT hr = IAudioClient_Initialize(audio->client, AUDCLNT_SHAREMODE_EXCLUSIVE, AUDCLNT_STREAMFLAGS_EVENTCALLBACK,
                                       period, period, &format, NULL);
  if (hr == AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED) {
    UINT32 size = 0;
    if (FAILED(IAudioClient_GetBufferSize(audio->client, &size))) return 0;
    period = (REFERENCE_TIME)(10000000.0 * size / 44100.0 + 0.5);

    SAFE_RELEASE(audio->client);

    if (FAILED(IMMDevice_Activate(audio->device, &IID_IAudioClient, CLSCTX_ALL, NULL, (void **)&audio->client))) return 0;
    if (FAILED(IAudioClient_Initialize(audio->client, AUDCLNT_SHAREMODE_EXCLUSIVE, AUDCLNT_STREAMFLAGS_EVENTCALLBACK,
                                       period, period, &format, NULL))) return 0;
  } else if (FAILED(hr)) return 0;

  if (!(audio->event = CreateEventEx(NULL, NULL, 0, EVENT_ALL_ACCESS))) return 0;
  if (FAILED(IAudioClient_SetEventHandle(audio->client, audio->event))) return 0;
  if (FAILED(IAudioClient_GetBufferSize(audio->client, &audio->buffer_size_in_frames))) return 0;
  if (FAILED(IAudioClient_GetService(audio->client, &IID_IAudioRenderClient, (void **)&audio->render_client))) return 0;

  audio->stream = CreateFile("data/flis_amnestia.raw", GENERIC_READ, 0, NULL, OPEN_EXISTING,
                             FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
  if (audio->stream == INVALID_HANDLE_VALUE) return 0;

  BYTE *ptr;
  if (FAILED(IAudioRenderClient_GetBuffer(audio->render_client, audio->buffer_size_in_frames, &ptr))) return 0;

  DWORD read_bytes;
  BOOL read_res = ReadFile(audio->stream, ptr, audio->buffer_size_in_frames * 4, &read_bytes, NULL);
  if (!read_res || read_bytes != audio->buffer_size_in_frames * 4) return 0;

  if (FAILED(IAudioRenderClient_ReleaseBuffer(audio->render_client, audio->buffer_size_in_frames, 0))) return 0;

  DWORD task_index = 0;
  HANDLE task = AvSetMmThreadCharacteristics(TEXT("Playback"), &task_index);
  if (task == NULL) return 0;

  if (FAILED(IAudioClient_Start(audio->client))) return 0;

  for (int i = 0; i < 10000; ++i) {
    WaitForSingleObject(audio->event, INFINITE);

    if (FAILED(IAudioRenderClient_GetBuffer(audio->render_client, audio->buffer_size_in_frames, &ptr))) return 0;

    read_res = ReadFile(audio->stream, ptr, audio->buffer_size_in_frames * 4, &read_bytes, NULL);
    if (!read_res || read_bytes != audio->buffer_size_in_frames * 4) return 0;
  ...
  if (FAILED(IAudioClient_Stop(audio->client))) return 0;
